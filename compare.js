...
function calculateCost(meeting, people, excludedNames) {
  ...
  let message = meeting.isRecurring
    ? "Total Cost per year"
    : maxMeetings > 1
      ? `Total Cost (${maxMeetings} sessions)`
      : "Cost";
  const breakdowns = [];
  ...
  for (let i = 0; i < people.length; i++) {
    ...
    const salMinute =
      salType === "hourly" ? people[i].salary / 60 : people[i].salary / 106000;
...
function renderMeetingList() {
  ...
  grid.innerHTML = meetings
    .map((m) => {
      const { totalResult, message } = calculateCost(
        m,
        people,
        m.excludedNames || [],
      );
      ...
      <div class="cmp-meeting-card card" onclick="openComparison(${m.id})">
        <div class="card-body">
          ...
          <div class="cmp-card-detail description">${m.durationMins} min · ${people.length} attendee${people.length !== 1 ? "s" : ""}</div>
...
function openComparison(meetingId) {
  ...
  // Populate controls
  document.getElementById("compare-meeting-name").textContent =
  ...
  if (selectedMeeting.isRecurring) {
    ...
    document.getElementById("cmp-times-per-month").value =
      selectedMeeting.timesPerMonth;
  } else {
    ...
    document.getElementById("cmp-total-occurrences").value =
...
function toggleCmpRecurring() {
  ...
  document.getElementById("cmp-recurring-fields").style.display = isRecurring
    ? "block"
    : "none";
  document.getElementById("cmp-one-time-fields").style.display = isRecurring
    ? "none"
    : "block";
  runComparison();
}
...
function buildScenarioMeeting() {
  ...
  const durationMins =
    rawDuration > 0 ? rawDuration : selectedMeeting.durationMins;
  const isRecurring = document.getElementById("cmp-recurring-yes").checked;
  ...
  const attendedCounts =
    origMax !== newMax ? {} : selectedMeeting.attendedCounts || {};
...
function renderCostPanel(costEl, tableEl, metaEl, meeting, people, excluded) {
  const { totalResult, message, breakdowns } = calculateCost(
    meeting,
    people,
    excluded,
  );
...
function runComparison() {
  ...
  const origResult = calculateCost(
    selectedMeeting,
    allPeople,
    selectedMeeting.excludedNames || [],
  ).totalResult;
  const scenResult = calculateCost(
    scenario,
    allPeople,
    scenarioExcluded,
  ).totalResult;
  ...
  const settingsChanged =
    ...
    (scenario.totalOccurrences || 1) !==
      (selectedMeeting.totalOccurrences || 1) ||
    JSON.stringify([...scenarioExcluded].sort()) !==
      JSON.stringify([...(selectedMeeting.excludedNames || [])].sort());
...
function saveScenario() {
  ...
  const newMax = scenario.isRecurring
    ? scenario.timesPerMonth * 12
    : scenario.totalOccurrences || 1;
